---
title: Yulong Liu
nav_title: About
nav: true
nav_order: 1
permalink: /
layout: page
hide_title: true
body_class: page-about
description: Ph.D. student in Earth and Atmospheric Sciences at Cornell University.
---

<section class="hero-card">
  <div class="hero-photo">
    <img src="{{ '/assets/img/yulong-liu-portrait.jpeg' | relative_url }}" alt="Portrait of Yulong Liu">
  </div>
  <div class="hero-copy">
    <p class="page-kicker">Yulong Liu</p>
    <h1>Ph.D. student in Earth and Atmospheric Sciences at Cornell University.</h1>
    <p class="hero-lead">
      I work across computational mechanics, implicit neural representation, poromechanics, and subsurface energy systems, with a focus on surrogate modeling for coupled multiphysics problems in Earth science. I am also increasingly interested in homogenization theory and physics-aware implicit neural representations.
    </p>

    <div class="tag-row">
      <span>Computational mechanics</span>
      <span>Implicit neural representation</span>
      <span>Poromechanics</span>
    </div>

    <div class="link-row">
      <a href="https://scholar.google.com/citations?user=0CV0lsQAAAAJ&hl=en" target="_blank" rel="noreferrer">Google Scholar</a>
      <a href="https://github.com/Yulongggggg" target="_blank" rel="noreferrer">GitHub</a>
      <a href="https://www.linkedin.com/in/yulong-liu-19451a322/" target="_blank" rel="noreferrer">LinkedIn</a>
      <a href="{{ '/assets/pdf/Yulong_CV2026_V2.pdf' | relative_url }}" target="_blank" rel="noreferrer">CV</a>
      <a href="mailto:yl3825@cornell.edu">Cornell email</a>
    </div>

    <dl class="fact-grid">
      <div class="fact-item">
        <dt>Program</dt>
        <dd>Ph.D. in Earth Sciences</dd>
      </div>
      <div class="fact-item">
        <dt>Advisor</dt>
        <dd><a class="advisor-link" href="https://arsonlab.engineering.cornell.edu/" target="_blank" rel="noreferrer">Chloé Arson</a></dd>
      </div>
      <div class="fact-item">
        <dt>Focus</dt>
        <dd>Computational mechanics · INR · Poromechanics</dd>
      </div>
      <div class="fact-item">
        <dt>Base</dt>
        <dd>Ithaca, New York</dd>
      </div>
      <div class="fact-item">
        <dt>Email</dt>
        <dd>yl3825@cornell.edu</dd>
      </div>
    </dl>
  </div>
</section>

<section class="section-stack">
  <div class="page-header">
    <p class="page-kicker">Overview</p>
    <h2 class="section-title">Current directions in my work.</h2>
  </div>
  <p class="section-copy">
    My current work sits at the intersection of continuum mechanics, scientific machine learning, and subsurface modeling. I am especially interested in building trustworthy computational tools that connect rigorous physical modeling with expressive data-driven representations.
  </p>
</section>
